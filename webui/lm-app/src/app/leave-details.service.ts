...
export class LeaveDetailsService {
  ...
  applyssLeave(leavedetails:LeaveDetails,empId : number):Observable<String> { 
    // alert("Emp ID " +empId);
    return this.http.post('http://localhost:8080/ftp85/api/LeaveDetailsRest/LDApplySSLeave/' +empId,leavedetails)
    .map(response=>response.text())
  }
